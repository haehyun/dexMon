...
# define ILOGH(...) ILOG(LOG_VERBOSE, __VA_ARGS__)
...
# define ILOGN(...) ((void)0)
# define ILOGC(...) do {                                                                              \
        if(gInterpPortable.codeCheck == false){                                                       \
            char debugStrBuf[256];                                                                    \
            int codelen = 0;                                                                          \
            int i = 0;                                                                                \
            snprintf(debugStrBuf, sizeof(debugStrBuf), __VA_ARGS__);                                  \
            if(gInterpPortable.currentPackageMode == PRINT_MODE &&                                    \
             gInterpPortable.codePrint == true &&                                                     \
             gInterpPortable.fp == fp){                                                               \
                    logging("%s", debugStrBuf);                                                      \
                    ++gInterpPortable.codeIndex;                                                      \
                    if(gInterpPortable.codeIndex == MAX_CODE_STREAM){                                 \
                        gInterpPortable.codePrint = false;                                            \
                        gInterpPortable.codeCheck = true;                                             \
                        ALOGE("msec|print complete");                                                 \
                    }                                                                                 \
            } else if(gInterpPortable.currentPackageMode == SANITIZE_SERVER_CORECODE_MODE &&          \
             gInterpPortable.codePrint == true &&                                                     \
             gInterpPortable.fp == fp){                                                               \
                codelen = strlen(debugStrBuf);                                                        \
                do{                                                                                   \
                    ALOGE("msec|Index : %d, Running code : %c, Original code : %c",                   \
                     gInterpPortable.codeIndex, debugStrBuf[i],                                       \
                     gInterpPortable.codeStream[gInterpPortable.codeIndex]);                          \
                    if(debugStrBuf[i++] != gInterpPortable.codeStream[gInterpPortable.codeIndex++]){  \
                        ALOGE("msec|TAMPER DETECT!! Injected routine is modified.");                  \
                        myClose(TAMPER_SERVER_CORECODE, self);                                        \
                    }                                                                                 \
                    if(gInterpPortable.codeIndex == gInterpPortable.codeSize){                        \
                        ALOGE("msec|Injected routine is unmodified.");                                \
                        gInterpPortable.codeCheck = true;                                             \
                    }                                                                                 \
                }while(i < codelen);                                                                  \
            }                                                                                         \
...
//if(gAndromonitormonitorOn == true){
...
# define ILOGH(...) do {                                                                              
        if(gAndromonitormonitorOn == true){                                                          
                ...
                snprintf(debugStrBuf, sizeof(debugStrBuf), __VA_ARGS__);                              
                if (curMethod != NULL){                                                               
                    logging("|%d|%4x|%d|%s", self->threadId, pc,                                      
                        self->systemTid, debugStrBuf);                                                
                }                                                                                     
...
# define ILOGH(...) do {                                                                          \
        if( (gAndromonitormonitorOn == true && gAndromonitor.extractInvokeOnly == false && (self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0)) ){    \
            char debugStrBuf[8192];                                                                \
            snprintf(debugStrBuf, sizeof(debugStrBuf), __VA_ARGS__);                              \
            if(curMethod != NULL){                                                                \
...
# define ILOGV(...)  do {                                                                         \
        if( (gAndromonitormonitorOn == true && (self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0)) ){     \
            char debugStrBuf[8192];                                                                \
            snprintf(debugStrBuf, sizeof(debugStrBuf), __VA_ARGS__);                              \
            if(curMethod != NULL){                                                                \
                logging("|%d|%s", self->threadId, debugStrBuf);                                   \
...
//    if(dvmThreadSelf()->threadId == 1){
        logging("|%d|%d| Registers (fp=%p):\n", dvmThreadSelf()->threadId, dvmThreadSelf()->systemTid,framePtr);
        for (i = method->registersSize-1; i >= 0; i--) {
            if (i >= localCount) {
                logging("|%d|%d|  v%-2d in%-2d : 0x%08x\n", dvmThreadSelf()->threadId,
                    dvmThreadSelf()->systemTid, i, i-localCount, framePtr[i]);
            } else {
                if (inOnly) {
                    logging("|%d|%d|  [...]\n", dvmThreadSelf()->threadId, dvmThreadSelf()->systemTid);
                    break;
                }
                const char* name = "";
    ...
    #endif
                logging("|%d|%d|  v%-2d      : 0x%08x %s\n",
                    dvmThreadSelf()->threadId, dvmThreadSelf()->systemTid, i, framePtr[i], name);
            }
        }
//    }
}
...
#define HANDLE_NUMCONV(_opcode, _opname, _fromtype, _totype)                \
    HANDLE_OPCODE(_opcode /*vA, vB*/)                                       \
        vdst = INST_A(inst);                                                \
        vsrc1 = INST_B(inst);                                               \
        ILOGH("%s v%d,v%d", (_opname), vdst, vsrc1);                        \
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));                 \
...
#define HANDLE_INT_TO_SMALL(_opcode, _opname, _type)                        \
    HANDLE_OPCODE(_opcode /*vA, vB*/)                                       \
        vdst = INST_A(inst);                                                \
        vsrc1 = INST_B(inst);                                               \
        ILOGH("int-to-%s v%d,v%d", (_opname), vdst, vsrc1);                 \
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));                 \
...
#define HANDLE_OP_CMPX(_opcode, _opname, _varType, _type, _nanVal)          \
    ...
    {                                                                       \
        ...
        vsrc1 = regs & 0xff;                                                \
        vsrc2 = regs >> 8;                                                  \
        ILOGH("cmp%s v%d,v%d,v%d", (_opname), vdst, vsrc1, vsrc2);          \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));                        \
...
#define HANDLE_OP_IF_XX(_opcode, _opname, _cmp)                             \
    HANDLE_OPCODE(_opcode /*vA, vB, +CCCC*/)                                \
        ...
        if ((s4) GET_REGISTER(vsrc1) _cmp (s4) GET_REGISTER(vsrc2)) {       \
            ...
            ILOGH("if-%s v%d,v%d,+0x%04x", (_opname), vsrc1, vsrc2,         \
                branchOffset);                                              \
            ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),    \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
            ILOGH("> branch taken");                                        \
            ...
            ILOGH("if-%s v%d,v%d,-", (_opname), vsrc1, vsrc2);              \
            ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),    \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
            FINISH(2);                                                      \
        }
...
#define HANDLE_OP_IF_XXZ(_opcode, _opname, _cmp)                            \
    HANDLE_OPCODE(_opcode /*vAA, +BBBB*/)                                   \
        ...
        if ((s4) GET_REGISTER(vsrc1) _cmp 0) {                              \
            int branchOffset = (s2)FETCH(1);    /* sign-extended */         \
            ILOGH("if-%s v%d,+0x%04x", (_opname), vsrc1, branchOffset);     \
            ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),    \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
            ILOGH("> branch taken");                                        \
            if (branchOffset < 0)                                           \
...
#define HANDLE_OP_X_INT(_opcode, _opname, _op, _chkdiv)                     \
    ...
    {                                                                       \
        ...
        vsrc1 = srcRegs & 0xff;                                             \
        vsrc2 = srcRegs >> 8;                                               \
        ILOGH("%s-int v%d,v%d", (_opname), vdst, vsrc1);                    \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        if (_chkdiv != 0) {                                                 \
...
#define HANDLE_OP_X_INT_LIT16(_opcode, _opname, _op, _chkdiv)               \
    HANDLE_OPCODE(_opcode /*vA, vB, #+CCCC*/)                               \
        ...
        vsrc1 = INST_B(inst);                                               \
        vsrc2 = FETCH(1);                                                   \
        ILOGH("%s-int/lit16 v%d,v%d,#+0x%04x",                              \
...
#define HANDLE_OP_X_INT_LIT8(_opcode, _opname, _op, _chkdiv)                \
    ...
    {                                                                       \
        ...
        ILOGH("%s-int/lit8 v%d,v%d,#+0x%02x",                               \
            (_opname), vdst, vsrc1, vsrc2);                                 \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        if (_chkdiv != 0) {                                                 \
...
#define HANDLE_OP_X_INT_2ADDR(_opcode, _opname, _op, _chkdiv)               \
    HANDLE_OPCODE(_opcode /*vA, vB*/)                                       \
        vdst = INST_A(inst);                                                \
        vsrc1 = INST_B(inst);                                               \
        ILOGH("%s-int-2addr v%d,v%d", (_opname), vdst, vsrc1);             \
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));                 \
        if (_chkdiv != 0) {                                                 \
            s4 firstVal, secondVal, result;                                 \
...
#define HANDLE_OP_X_LONG(_opcode, _opname, _op, _chkdiv)                    \
    ...
    {                                                                       \
        ...
        vsrc1 = srcRegs & 0xff;                                             \
        vsrc2 = srcRegs >> 8;                                               \
        ILOGH("%s-long v%d,v%d,v%d", (_opname), vdst, vsrc1, vsrc2);       \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        if (_chkdiv != 0) {                                                 \
...
#define HANDLE_OP_X_LONG_2ADDR(_opcode, _opname, _op, _chkdiv)              \
    HANDLE_OPCODE(_opcode /*vA, vB*/)                                       \
        vdst = INST_A(inst);                                                \
        vsrc1 = INST_B(inst);                                               \
        ILOGH("%s-long-2addr v%d,v%d", (_opname), vdst, vsrc1);            \
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));                 \
        if (_chkdiv != 0) {                                                 \
            s8 firstVal, secondVal, result;                                 \
...
#define HANDLE_OP_X_FLOAT(_opcode, _opname, _op)                            \
    ...
    {                                                                       \
        ...
        vsrc1 = srcRegs & 0xff;                                             \
        vsrc2 = srcRegs >> 8;                                               \
        ILOGH("%s-float v%d,v%d,v%d", (_opname), vdst, vsrc1, vsrc2);      \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
...
#define HANDLE_OP_X_DOUBLE(_opcode, _opname, _op)                           \
    ...
    {                                                                       \
        ...
        vsrc1 = srcRegs & 0xff;                                             \
        vsrc2 = srcRegs >> 8;                                               \
        ILOGH("%s-double v%d,v%d,v%d", (_opname), vdst, vsrc1, vsrc2);     \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
...
#define HANDLE_OP_X_FLOAT_2ADDR(_opcode, _opname, _op)                      \
    HANDLE_OPCODE(_opcode /*vA, vB*/)                                       \
        vdst = INST_A(inst);                                                \
        vsrc1 = INST_B(inst);                                               \
        ILOGH("%s-float-2addr v%d,v%d", (_opname), vdst, vsrc1);           \
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));                 \
        SET_REGISTER_FLOAT(vdst,                                            \
...
#define HANDLE_OP_X_DOUBLE_2ADDR(_opcode, _opname, _op)                     \
    HANDLE_OPCODE(_opcode /*vA, vB*/)                                       \
        vdst = INST_A(inst);                                                \
        vsrc1 = INST_B(inst);                                               \
        ILOGH("%s-double-2addr v%d,v%d", (_opname), vdst, vsrc1);          \
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));                 \
        SET_REGISTER_DOUBLE(vdst,                                           \
...
#define HANDLE_OP_AGET(_opcode, _opname, _type, _regsize)                   \
    ...
    {                                                                       \
        ...
        ILOGH("aget%s v%d,v%d,v%d", (_opname), vdst, vsrc1, vsrc2);        \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        ...
        ILOGH("+ AGET[%d]=%#x", GET_REGISTER(vsrc2), GET_REGISTER(vdst));   \
    }                                                                       \
    FINISH(2);
...
#define HANDLE_OP_APUT(_opcode, _opname, _type, _regsize)                   \
    ...
    {                                                                       \
        ...
        ILOGH("aput%s v%d,v%d,v%d", (_opname), vdst, vsrc1, vsrc2);        \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        arrayObj = (ArrayObject*) GET_REGISTER(vsrc1);                      \
        if (!checkForNull((Object*) arrayObj))                              \
        ...
        ILOGH("+ APUT[%d]=0x%08x", GET_REGISTER(vsrc2), GET_REGISTER(vdst));\
        ((_type*)(void*)arrayObj->contents)[GET_REGISTER(vsrc2)] =          \
...
#define HANDLE_IGET_X(_opcode, _opname, _ftype, _regsize)                   \
    ...
    {                                                                       \
        ...
        ILOGH("iget%s v%d,v%d,field@0x%04x", (_opname), vdst, vsrc1, ref);  \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),           \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        ...
        ILOGH("+ IGET '%s'=0x%08llx", ifield->name,                         \
...
#define HANDLE_IGET_X_QUICK(_opcode, _opname, _ftype, _regsize)             \
    ...
    {                                                                       \
        ...
        ILOGH("iget%s-quick v%d,v%d,field@+%u",                             \
...
#define HANDLE_IPUT_X(_opcode, _opname, _ftype, _regsize)                   \
    ...
    {                                                                       \
        ...
        vsrc1 = INST_B(inst);   /* object ptr */                            \
        ref = FETCH(1);         /* field ref */                             \
        ILOGH("iput%s v%d,v%d,field@0x%04x", (_opname), vdst, vsrc1, ref);  \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),           \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        obj = (Object*) GET_REGISTER(vsrc1);                                \
        if (!checkForNull(obj))                                             \
        ...
        dvmSetField##_ftype(obj, ifield->byteOffset,                        \
            GET_REGISTER##_regsize(vdst));                                  \
        ILOGH("+ IPUT '%s'=0x%08llx", ifield->name,                         \
            (u8) GET_REGISTER##_regsize(vdst));                             \
    }                                                                       \
    FINISH(2);
...
#define HANDLE_IPUT_X_QUICK(_opcode, _opname, _ftype, _regsize)             \
    ...
    {                                                                       \
        ...
        vsrc1 = INST_B(inst);   /* object ptr */                            \
        ref = FETCH(1);         /* field offset */                          \
        ILOGH("iput%s-quick v%d,v%d,field@0x%04x",                          \
            (_opname), vdst, vsrc1, ref);                                   \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),           \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        obj = (Object*) GET_REGISTER(vsrc1);                                \
 ...
 */
/*        ILOGH("sget%s v%d,sfield@0x%04x", (_opname), vdst, ref);           
*/
#define HANDLE_SGET_X(_opcode, _opname, _ftype, _regsize)                   \
    ...
    {                                                                       \
        ...
        ILOGH("sget%s v%d,%s->%s:%s sfield@0x%04x", (_opname), vdst,         \
            sfield->clazz->descriptor,sfield->name, sfield->signature, ref); \
        ILOGH("+ SGET '%s'=0x%08llx", sfield->name, (u8)GET_REGISTER##_regsize(vdst)); \
    }                                                                       \
    FINISH(2);
...
#define HANDLE_SPUT_X(_opcode, _opname, _ftype, _regsize)                   \
    ...
    {                                                                       \
        ...
        vdst = INST_AA(inst);                                               \
        ref = FETCH(1);         /* field ref */                             \
        ILOGH("sput%s v%d,sfield@0x%04x", (_opname), vdst, ref);           \
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),        \
                INST_INST(FETCH(1)), INST_AA(FETCH(1)));                    \
        sfield = (StaticField*)dvmDexGetResolvedField(methodClassDex, ref); \
        if (sfield == NULL) {                                               \
...
void dvmInterpretPortable(Thread* self)
{
    ...
    //////////haehyun
    bool gAndromonitormonitorOn = false;
...
/////// haehyun
    if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0) {
        char* desc_v;
        desc_v = dexProtoCopyMethodDescriptor(&curMethod->prototype);
        if(gAndromonitor.currentMonitorMode == FULL_MODE) {
            gAndromonitormonitorOn = true;
            ILOGV("Entering interpreter loop");
            char* desc_loop;
            desc_loop = dexProtoCopyMethodDescriptor(&curMethod->prototype);
            ILOGV("[fp=%p] %s->%s%s", fp, curMethod->clazz->descriptor, curMethod->name, desc_loop);
            free(desc_loop);
        }
        if(gAndromonitor.currentMonitorMode == CLASS_MODE) {
            if(gAndromonitormonitorOn == false){
                if(strncmp(curMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0) {
                    gAndromonitormonitorOn = true;
                    ILOGV("Entering interpreter loop");
                    char* desc_loop;
                    desc_loop = dexProtoCopyMethodDescriptor(&curMethod->prototype);
        ...
        if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
            if(gAndromonitormonitorOn == false){
                if(strncmp(curMethod->clazz->descriptor, gAndromonitor.classDescriptor, strlen(curMethod->clazz->descriptor)) == 0) {
                    if(strncmp(curMethod->name, gAndromonitor.methodName, strlen(curMethod->name)) == 0){
                        if(strncmp(desc_v, gAndromonitor.methodDescriptor, strlen(desc_v)) == 0){
                            gAndromonitormonitorOn = true;
                            ILOGV("Entering interpreter loop");
                            char* desc_loop;
                            desc_loop = dexProtoCopyMethodDescriptor(&curMethod->prototype);
        ...
        free(desc_v);
////mj
        if(gAndromonitor.isNative == true) {
            gAndromonitormonitorOn = true;
            char* desc_loop;
            desc_loop = dexProtoCopyMethodDescriptor(&curMethod->prototype);
            ILOGV("[fp=%p][pc=%#x] %s->%s%s", fp, pc - curMethod->insns,
                    curMethod->clazz->descriptor, curMethod->name, desc_loop);
            free(desc_loop);
            gAndromonitormonitorOn = false;
        }
...
HANDLE_OPCODE(OP_MOVE /*vA, vB*/)
    vdst = INST_A(inst);
    vsrc1 = INST_B(inst);
    ILOGH("move%s v%d,v%d %s(v%d=0x%08x)",
...
HANDLE_OPCODE(OP_MOVE_FROM16 /*vAA, vBBBB*/)
    vdst = INST_AA(inst);
    vsrc1 = FETCH(1);
    ILOGH("move%s/from16 v%d,v%d %s(v%d=0x%08x)",
...
HANDLE_OPCODE(OP_MOVE_16 /*vAAAA, vBBBB*/)
    vdst = FETCH(1);
    vsrc1 = FETCH(2);
    ILOGH("move%s/16 v%d,v%d %s(v%d=0x%08x)",
        (INST_INST(inst) == OP_MOVE_16) ? "" : "-object", vdst, vsrc1,
        kSpacing, vdst, GET_REGISTER(vsrc1));
    ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
    SET_REGISTER(vdst, GET_REGISTER(vsrc1));
    FINISH(3);
OP_END
...
HANDLE_OPCODE(OP_MOVE_WIDE /*vA, vB*/)
    ...
    vdst = INST_A(inst);
    vsrc1 = INST_B(inst);
    ILOGH("move-wide v%d,v%d %s(v%d=0x%08llx)", vdst, vsrc1,
...
HANDLE_OPCODE(OP_MOVE_WIDE_FROM16 /*vAA, vBBBB*/)
    vdst = INST_AA(inst);
    vsrc1 = FETCH(1);
    ILOGH("move-wide/from16 v%d,v%d  (v%d=0x%08llx)", vdst, vsrc1,
...
HANDLE_OPCODE(OP_MOVE_WIDE_16 /*vAAAA, vBBBB*/)
    vdst = FETCH(1);
    vsrc1 = FETCH(2);
    ILOGH("move-wide/16 v%d,v%d %s(v%d=0x%08llx)", vdst, vsrc1,
...
HANDLE_OPCODE(OP_MOVE_RESULT /*vAA*/)
    vdst = INST_AA(inst);
    ILOGH("move-result%s v%d %s(v%d=0x%08x)",
...
HANDLE_OPCODE(OP_MOVE_RESULT_WIDE /*vAA*/)
    vdst = INST_AA(inst);
    ILOGH("move-result-wide v%d %s(0x%08llx)", vdst, kSpacing, retval.j);
    ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
...
HANDLE_OPCODE(OP_CONST_4 /*vA, #+B*/)
    {
        ...
        ILOGH("const/4 v%d,#0x%02x", vdst, (s4)tmp);
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
...
HANDLE_OPCODE(OP_CONST_16 /*vAA, #+BBBB*/)
    vdst = INST_AA(inst);
    vsrc1 = FETCH(1);
    ILOGH("const/16 v%d,#0x%04x", vdst, (s2)vsrc1);
    ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
        INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_CONST /*vAA, #+BBBBBBBB*/)
    {
        ...
        tmp = FETCH(1);
        tmp |= (u4)FETCH(2) << 16;
        ILOGH("const v%d,#0x%08x", vdst, tmp);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
        SET_REGISTER(vdst, tmp);
...
HANDLE_OPCODE(OP_CONST_HIGH16 /*vAA, #+BBBB0000*/)
    vdst = INST_AA(inst);
    vsrc1 = FETCH(1);
    ILOGH("const/high16 v%d,#0x%04x0000", vdst, vsrc1);
    ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
        INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_CONST_WIDE_16 /*vAA, #+BBBB*/)
    vdst = INST_AA(inst);
    vsrc1 = FETCH(1);
    ILOGH("const-wide/16 v%d,#0x%04x", vdst, (s2)vsrc1);
    ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
        INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_CONST_WIDE_32 /*vAA, #+BBBBBBBB*/)
    {
        ...
        tmp = FETCH(1);
        tmp |= (u4)FETCH(2) << 16;
        ILOGH("const-wide/32 v%d,#0x%08x", vdst, tmp);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
...
HANDLE_OPCODE(OP_CONST_WIDE /*vAA, #+BBBBBBBBBBBBBBBB*/)
    {
        ...
        tmp |= (u8)FETCH(3) << 32;
        tmp |= (u8)FETCH(4) << 48;
        ILOGH("const-wide v%d,#0x%08llx", vdst, tmp);
        ILOGC("%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)),
            INST_INST(FETCH(3)), INST_AA(FETCH(3)),
            INST_INST(FETCH(4)), INST_AA(FETCH(4)));
...
HANDLE_OPCODE(OP_CONST_WIDE_HIGH16 /*vAA, #+BBBB000000000000*/)
    vdst = INST_AA(inst);
    vsrc1 = FETCH(1);
    ILOGH("const-wide/high16 v%d,#0x%04x000000000000", vdst, vsrc1);
    ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
        INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_CONST_STRING /*vAA, string@BBBB*/)
    {
        ...
        tempstr = dvmCreateCstrFromString(strObj);
        ILOGH("const-string v%d string@0x%04x, string = %s", vdst, ref, tempstr);
        ILOGC("%02x%02x%02x%02x%s", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)), tempstr);

        /*if(strcmp(tempstr,"startextr")==0){
            gInterpPortable.callmyprint = true;
            gInterpPortable.fp = fp;
            gInterpPortable.pDexFile = curMethod->clazz->pDvmDex->pDexFile;
            ALOGE("msec|startextr");
        }

        if(strcmp(tempstr,"stopextr")==0){
            gInterpPortable.callmyprint = false;   
            gInterpPortable.fp = NULL;
            gInterpPortable.pDexFile = NULL;
            ALOGE("msec|stopextr");
        }

        if(strcmp(tempstr,"startextrfull")==0){
            gInterpPortable.callmyprintfull = true;
            gInterpPortable.fp = fp;
            gInterpPortable.pDexFile = curMethod->clazz->pDvmDex->pDexFile;
            ALOGE("msec|startextrfull");
        }

        if(strcmp(tempstr,"stopextrfull")==0){
            gInterpPortable.callmyprintfull = false;
            gInterpPortable.fp = NULL;
            gInterpPortable.pDexFile = NULL;
            ALOGE("msec|stopextrfull");
        }
        
        if(strcmp(tempstr,"killthisapp")==0){
            ALOGE("msec|killthisapp");
            myClose(0, self);
        }

        if(strcmp(tempstr, "startattest")==0){
        gInterpPortable.codePrint = true;
        gInterpPortable.fp = fp;
        ALOGE("msec|startattest");
        }

        if(strcmp(tempstr, "stopattest")==0){
        gInterpPortable.codePrint = false;
        ALOGE("msec|stopattest");
        }*/
...
HANDLE_OPCODE(OP_CONST_STRING_JUMBO /*vAA, string@BBBBBBBB*/)
    {
        ...
        tmp = FETCH(1);
        tmp |= (u4)FETCH(2) << 16;
        //ILOGH("const-string/jumbo v%d string@0x%08x", vdst, tmp);
        ...
        tempstr = dvmCreateCstrFromString(strObj);
        ILOGH("const-string/jumbo v%d string@0x%08x, string = %s", vdst, tmp, tempstr);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
        SET_REGISTER(vdst, (u4) strObj);
...
HANDLE_OPCODE(OP_CONST_CLASS /*vAA, class@BBBB*/)
    {
        ...
        vdst = INST_AA(inst);
        ref = FETCH(1);
        ILOGH("const-class v%d class@0x%04x", vdst, ref);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_MONITOR_ENTER /*vAA*/)
    {
        ...
        vsrc1 = INST_AA(inst);
        ILOGH("monitor-enter v%d %s(0x%08x)",
            vsrc1, kSpacing+6, GET_REGISTER(vsrc1));
        obj = (Object*)GET_REGISTER(vsrc1);
        if (!checkForNullExportPC(obj, fp, pc))
            GOTO_exceptionThrown();
        ILOGH("+ locking %p %s", obj, obj->clazz->descriptor);
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
        EXPORT_PC();    /* need for precise GC */
...
HANDLE_OPCODE(OP_MONITOR_EXIT /*vAA*/)
    {
        ...
        vsrc1 = INST_AA(inst);
        ILOGH("monitor-exit v%d %s(0x%08x)",
        ...
        ILOGH("+ unlocking %p %s", obj, obj->clazz->descriptor);
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
...
HANDLE_OPCODE(OP_CHECK_CAST /*vAA, class@BBBB*/)
    {
        ...
        ILOGH("check-cast v%d,class@0x%04x", vsrc1, ref);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_INSTANCE_OF /*vA, vB, class@CCCC*/)
    {
        ...
        ref = FETCH(1);         /* class to check against */
        ILOGH("instance-of v%d,v%d,class@0x%04x", vdst, vsrc1, ref);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_ARRAY_LENGTH /*vA, vB*/)
    {
        ...
        vsrc1 = INST_B(inst);
        arrayObj = (ArrayObject*) GET_REGISTER(vsrc1);
        ILOGH("array-length v%d,v%d  (%p)", vdst, vsrc1, arrayObj);
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
...
HANDLE_OPCODE(OP_NEW_INSTANCE /*vAA, class@BBBB*/)
    {
        ...
        //ILOGH("new-instance v%d,class@0x%04x", vdst, ref);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
        INST_INST(FETCH(1)), INST_AA(FETCH(1)));
        clazz = dvmDexGetResolvedClass(methodClassDex, ref);
...
//////////////////////////////////////////////////////////////////////////////           
        tempObj = dvmAllocObject(clazz, ALLOC_DONT_TRACK);
        if (tempObj != NULL)
            ILOGH("new-instance v%d,%s [class@0x%04x]", vdst, tempObj->clazz->descriptor,ref);
        else
            ILOGH("new-instance v%d,class@0x%04x", vdst, ref);
//////////////////////////////////////////////////////////////////////////////           
...
HANDLE_OPCODE(OP_NEW_ARRAY /*vA, vB, class@CCCC*/)
    {
        ...
        ref = FETCH(1);
        // ILOGH("new-array v%d,v%d,class@0x%04x  (%d elements)",
...
//////////////////////////////////////////////////////////////////////////////           
        ILOGH("new-array v%d,v%d,%s  [class@0x%04x] (%d elements)",
            vdst, vsrc1, arrayClass->descriptor, ref, (s4) GET_REGISTER(vsrc1));
//////////////////////////////////////////////////////////////////////////////           
...
HANDLE_OPCODE(OP_FILL_ARRAY_DATA)   /*vAA, +BBBBBBBB*/
    {
        ...
        ILOGH("fill-array-data v%d +0x%04x", vsrc1, offset);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
...
HANDLE_OPCODE(OP_THROW /*vAA*/)
    {
        ...
        vsrc1 = INST_AA(inst);
        ILOGH("throw v%d  (%p)", vsrc1, (void*)GET_REGISTER(vsrc1));
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
        obj = (Object*) GET_REGISTER(vsrc1);
        if (!checkForNull(obj)) {
...
HANDLE_OPCODE(OP_PACKED_SWITCH /*vAA, +BBBB*/)
    {
        ...
        vsrc1 = INST_AA(inst);
        offset = FETCH(1) | (((s4) FETCH(2)) << 16);
        ILOGH("packed-switch v%d +0x%04x", vsrc1, offset);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
        ...
        ILOGH("> branch taken (0x%04x)", offset);
...
HANDLE_OPCODE(OP_SPARSE_SWITCH /*vAA, +BBBB*/)
    {
        ...
        vsrc1 = INST_AA(inst);
        offset = FETCH(1) | (((s4) FETCH(2)) << 16);
        ILOGH("sparse-switch v%d +0x%04x", vsrc1, offset);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
        switchData = pc + offset;       // offset in 16-bit units
...
HANDLE_OPCODE(OP_APUT_OBJECT /*vAA, vBB, vCC*/)
    {
        ...
        vsrc1 = arrayInfo & 0xff;   /* BB: array ptr */
        vsrc2 = arrayInfo >> 8;     /* CC: index */
        ILOGH("aput%s v%d,v%d,v%d", "-object", vdst, vsrc1, vsrc2);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
        arrayObj = (ArrayObject*) GET_REGISTER(vsrc1);
        if (!checkForNull((Object*) arrayObj))
...
HANDLE_OPCODE(OP_REM_FLOAT /*vAA, vBB, vCC*/)
    {
        ...
        vsrc1 = srcRegs & 0xff;
        vsrc2 = srcRegs >> 8;
        ILOGH("%s-float v%d,v%d,v%d", "mod", vdst, vsrc1, vsrc2);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
        SET_REGISTER_FLOAT(vdst,
...
HANDLE_OPCODE(OP_REM_DOUBLE /*vAA, vBB, vCC*/)
    {
        ...
        vsrc1 = srcRegs & 0xff;
        vsrc2 = srcRegs >> 8;
        ILOGH("%s-double v%d,v%d,v%d", "mod", vdst, vsrc1, vsrc2);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
        SET_REGISTER_DOUBLE(vdst,
...
HANDLE_OPCODE(OP_REM_FLOAT_2ADDR /*vA, vB*/)
    vdst = INST_A(inst);
    vsrc1 = INST_B(inst);
    ILOGH("%s-float-2addr v%d,v%d", "mod", vdst, vsrc1);
    ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
    SET_REGISTER_FLOAT(vdst,
...
HANDLE_OPCODE(OP_REM_DOUBLE_2ADDR /*vA, vB*/)
    vdst = INST_A(inst);
    vsrc1 = INST_B(inst);
    ILOGH("%s-double-2addr v%d,v%d", "mod", vdst, vsrc1);
    ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
    SET_REGISTER_DOUBLE(vdst,
...
HANDLE_OPCODE(OP_RSUB_INT /*vA, vB, #+CCCC*/)
    {
        ...
        vsrc1 = INST_B(inst);
        vsrc2 = FETCH(1);
        ILOGH("rsub-int v%d,v%d,#+0x%04x", vdst, vsrc1, vsrc2);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_RSUB_INT_LIT8 /*vAA, vBB, #+CC*/)
    {
        ...
        vsrc1 = litInfo & 0xff;
        vsrc2 = litInfo >> 8;
        ILOGH("%s-int/lit8 v%d,v%d,#+0x%02x", "rsub", vdst, vsrc1, vsrc2);
        ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_BREAKPOINT)
    {
        ...
        ILOGV("+++ break 0x%02x (0x%04x -> 0x%04x)", originalOpcode, inst,
            INST_REPLACE_OP(inst, originalOpcode));
        ILOGC("%02x%02x", INST_INST(inst), INST_AA(inst));
...
HANDLE_OPCODE(OP_THROW_VERIFICATION_ERROR)
    ...
    ILOGV("OP_THROW_VERIFICATION_ERROR");
    ILOGC("%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
        INST_INST(FETCH(1)), INST_AA(FETCH(1)));
...
HANDLE_OPCODE(OP_EXECUTE_INLINE /*vB, {vD, vE, vF, vG}, inline@CCCC*/)
    {
        ...
        // ILOGV("execute-inline args=%d @%d {regs=0x%04x}",
            // vsrc1, ref, vdst);
/*//////haehyun
        if(gInterpPortable.currentPackageMode == MONITOR_MODE && gInterpPortable.callmyprintfull == false){
            //if(strstr(methodToCall->clazz->descriptor, "example/msec") != NULL && gInterpPortable.callmyprintfull == false){
            if(strstr(methodToCall->clazz->descriptor, "example/msec") != NULL){
                gInterpPortable.callmyprintfull = true;
            }
        }
*////////////
        ILOGV("execute-inline args=%d @%d {v%d, v%d, v%d, v%d}",
            vsrc1, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12);
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
...
HANDLE_OPCODE(OP_INVOKE_OBJECT_INIT_RANGE /*{vCCCC..v(CCCC+AA-1)}, meth@BBBB*/)
    {
        ...
        if (self->interpBreak.ctl.subMode & kSubModeDebuggerActive) {
            ...
            ILOGV("OP_INVOKE_OBJECT_INIT_RANGE");
            ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
                INST_INST(FETCH(1)), INST_AA(FETCH(1)),
                INST_INST(FETCH(2)), INST_AA(FETCH(2)));
        }
...
GOTO_TARGET(filledNewArray, bool methodCallRange, bool)
    {
        ...
        if (methodCallRange) {
            ...
            ILOGH("filled-new-array-range args=%d @0x%04x {regs=v%d-v%d}",
                vsrc1, ref, vdst, vdst+vsrc1-1);
        } else {
            arg5 = INST_A(inst);
            vsrc1 = INST_B(inst);   /* #of elements */
            // ILOGH("filled-new-array args=%d @0x%04x {regs=0x%04x %x}",
            //    vsrc1, ref, vdst, arg5);
            ILOGH("filled-new-array args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
               vsrc1, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, arg5);
        }
        ILOGC("%02x%02x%02x%02x%02x%02x", INST_INST(inst), INST_AA(inst),
            INST_INST(FETCH(1)), INST_AA(FETCH(1)),
            INST_INST(FETCH(2)), INST_AA(FETCH(2)));
...
#endif
         //char* desc_v;
        // desc_v = dexProtoCopyMethodDescriptor(&methodToCall->prototype);
        //         if(strcmp(methodToCall->clazz->descriptor, "Lcom/example/msec/SecretClass;") == 0 ){
        //             if(strcmp(methodToCall->name, "secretMethod") == 0){
        //                 if(strcmp(desc_v, "()Z") == 0){
/////// haehyun
        if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0) {
            ...
            if(gAndromonitor.currentMonitorMode == CLASS_MODE) {
                if(gAndromonitormonitorOn == false){
                    if(strncmp(methodToCall->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0) {
                        gAndromonitormonitorOn = true;
                    }
                }
            }
            if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
                if(gAndromonitormonitorOn == false){
                    if(strncmp(methodToCall->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0) {
                        if(strncmp(methodToCall->name, gAndromonitor.methodName, gAndromonitor.length_MN) == 0){
                            if(strncmp(desc_v, gAndromonitor.methodDescriptor, gAndromonitor.length_MD) == 0){
                                gAndromonitormonitorOn = true;
                            }
                        }
                    }
...
////////////
        ...
        */
        if (methodCallRange) {
            ...
            assert((vsrc1>>4) > 0);
            ILOGV("invoke-virtual args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
                vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
        }
...
*////////////
        ...
        //    ILOGV("invoke-super args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
        //        vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
...
*////////////
        ...
        //        vsrc1, ref, vdst, vdst+vsrc1-1);
            ...
            // ILOGV("invoke-interface args=%d @0x%04x {regs=0x%04x %x}",
                // vsrc1 >> 4, ref, vdst, vsrc1 & 0x0f);
        //    ILOGV("invoke-interface args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
        //        vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
...
GOTO_TARGET(invokeDirect, bool methodCallRange)
    {
        ...
        if (methodCallRange) {
            ...
            // ILOGV("invoke-direct args=%d @0x%04x {regs=0x%04x %x}",
                // vsrc1 >> 4, ref, vdst, vsrc1 & 0x0f);
            //ILOGV("invoke-direct args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
            //    vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
            thisReg = vdst & 0x0f;
        }
...
////////////
    ...
    if (methodCallRange)
        ...
        // ILOGV("invoke-static args=%d @0x%04x {regs=0x%04x %x}",
            // vsrc1 >> 4, ref, vdst, vsrc1 & 0x0f);
        ILOGV("invoke-static args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
            vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
...
GOTO_TARGET(invokeVirtualQuick, bool methodCallRange)
    {
        ...
        //        vsrc1, ref, vdst, vdst+vsrc1-1);
            ...
            // ILOGV("invoke-virtual-quick args=%d @0x%04x {regs=0x%04x %x}",
                // vsrc1 >> 4, ref, vdst, vsrc1 & 0x0f);
        //    ILOGV("invoke-virtual-quick args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
        //        vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
...
GOTO_TARGET(invokeSuperQuick, bool methodCallRange)
    {
        ...
        if (methodCallRange) {
            ...
            // ILOGV("invoke-super-quick args=%d @0x%04x {regs=0x%04x %x}",
                // vsrc1 >> 4, ref, vdst, vsrc1 & 0x0f);
            //ILOGV("invoke-super-quick args=%d @0x%04x {v%d, v%d, v%d, v%d, v%d}",
            //    vsrc1 >> 4, ref, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
...
GOTO_TARGET(returnFromMethod)
    {
        ...
        StringObject* tempstrObj;
        const Method* tempMethod = curMethod;
...
////////////// Haehyun
            if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0) {
                desc_r = dexProtoCopyMethodDescriptor(&tempMethod->prototype);
                ...
                if(gAndromonitor.currentMonitorMode == CLASS_MODE) {
                    if(gAndromonitormonitorOn == false){
                        if(strncmp(tempMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0){
                            gAndromonitormonitorOn = true;
                        }
                    }
                }
                if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
                    if(gAndromonitormonitorOn == false){
                        if(strncmp(tempMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0){
                            if(strncmp(tempMethod->name, gAndromonitor.methodName, gAndromonitor.length_MN) == 0){
                                if(strncmp(desc_r, gAndromonitor.methodDescriptor, gAndromonitor.length_MD) == 0){
                                    gAndromonitormonitorOn = true;
                                }
                            }
                        }
                ...
                if(strstr(desc_r, ")Ljava/lang/String;")!=NULL){
                    tempstrObj = (StringObject*)retval.i;
                    ILOGV("retStr = %s", dvmCreateCstrFromString(tempstrObj));
                }
                else{
                    ILOGV("retval=0x%llx", retval.j);
                }
                free(desc_r);
            }
///////////haehyun
            ...
            ILOGV("+++ returned into break frame [fp = %p]", fp);
            GOTO_bail();
        }
...
/////// haehyun
        if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0) {
            ...
            desc_r = dexProtoCopyMethodDescriptor(&curMethod->prototype);
            ...
            if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
                if(gAndromonitormonitorOn == false){
                    if(strncmp(curMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0){
                        if(strncmp(curMethod->name, gAndromonitor.methodName, gAndromonitor.length_MN) == 0){
                            if(strncmp(desc_r, gAndromonitor.methodDescriptor, gAndromonitor.length_MD) == 0){
                                gAndromonitormonitorOn = true;
                            }
                        }
            ...
            desc_vr = dexProtoCopyMethodDescriptor(&tempMethod->prototype);
            
            if(strstr(desc_vr, ")Ljava/lang/String;")!=NULL){
                tempstrObj = (StringObject*)retval.i;
                ILOGV("retStr = %s", dvmCreateCstrFromString(tempstrObj));
            }
            else{
            ...
            if(gAndromonitor.extractInvokeOnly == false){
                ILOGV("return to %s->%s%s [fp=%p]\n", curMethod->clazz->descriptor,
                    curMethod->name, desc_r, fp);
            }
            else{
                ILOGV("return to %s->%s%s\n", curMethod->clazz->descriptor,
                    curMethod->name, desc_r);
            }
...
//////////////Haehyun
        if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0) {
            desc_vr = dexProtoCopyMethodDescriptor(&curMethod->prototype);
            if(gAndromonitormonitorOn == true) {
                if(gAndromonitor.currentMonitorMode == CLASS_MODE) {
                    if(strncmp(curMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) != 0){
                        gAndromonitormonitorOn = false;
                ...
                if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
                    if((strncmp(curMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) != 0) ||
                       (strncmp(curMethod->name, gAndromonitor.methodName, gAndromonitor.length_MN) != 0) ||
                       (strncmp(desc_vr, gAndromonitor.methodDescriptor, gAndromonitor.length_MD) != 0) ) {
                            gAndromonitormonitorOn = false;
                    }
                }
            }
...
GOTO_TARGET(exceptionThrown)
    {
        ...
        ILOGV("Handling exception %s at %s:%d",
        ...
        if (catchRelPc < 0) {
            ...
            ILOGV("Exception %s from %s:%d not caught locally",
...
#if DVM_SHOW_EXCEPTION >= 3
        {
            ...
            ILOGV("Exception %s thrown from %s:%d to %s:%d",
                exception->clazz->descriptor, dvmGetMethodSourceFile(curMethod),
                dvmLineNumFromPC(curMethod, pc - curMethod->insns),
        ...
        char* desc_x;
        desc_x = dexProtoCopyMethodDescriptor(&curMethod->prototype);

        if(gAndromonitor.extractInvokeOnly == false){
            ILOGV("[fp=%p] %s->%s%s (from exceptionThrown)", fp, curMethod->clazz->descriptor,
                curMethod->name, desc_x);
        }
        else{
            ILOGV(" %s->%s%s (from exceptionThrown) \n", curMethod->clazz->descriptor,
                curMethod->name, desc_x);
        }
...
GOTO_TARGET_END
     ...
     *  "vsrc1" holds the argument llcount (4 bits) and the 5th argument index
...
////////////////////////////////////////////output_deobfParam
        if( (gAndromonitor.extractParam && gAndromonitormonitorOn == true) ){
            if(methodCallRange) {
                //ILOGV("args=%d {regs=v%d-v%d}", vsrc1, vdst, vdst+vsrc1-1);
                //haehyun Check for existence of 'this' pointer
                ...
                ILOGV("paramCnt : %d, args %d", paramCnt, vsrc1);

                
                //haehyun Print the parameters
                
                paramStrIdx = desc_v;
                paramStrIdx++;
                if(paramCnt != vsrc1) {
                    ILOGV("param1 %s thisPtr", ((Object*)GET_REGISTER(vdst))->clazz->descriptor);
                    paramNum = 2;
                } else {
                    paramNum = 1;
                }

                for(; vdst+paramNum-1 <= vdst+vsrc1-1; ++paramNum) {
                    // ILOGV("deobfPararm %d", paramNum);
                    registerNum = vdst+paramNum-1;
                    switch(*paramStrIdx) {
                    case 'Z':
                        ILOGV("param%d Z %u", paramNum, (u1)GET_REGISTER(registerNum));
                        break;
                    case 'B':
                        ILOGV("param%d B %d", paramNum, (s1)GET_REGISTER(registerNum));
                        break;
                    case 'C':
                        utf8Str = (char*)malloc(5);
                        utf16Str = (u2*)&(GET_REGISTER(registerNum));
                        deobfConvertUtf16ToUtf8(utf8Str, utf16Str, 1); 
                        ILOGV("param%d C %s", paramNum, utf8Str);
                        free(utf8Str);
                        break;
                    case 'S':
                        ILOGV("param%d S %d", paramNum, (s2)GET_REGISTER(registerNum));
                        break;
                    case 'I':
                        ILOGV("param%d I %d", paramNum, (s4)GET_REGISTER_INT(registerNum));
                        break;
                    case 'J':
                        ILOGV("param%d J %lld", paramNum, (s8)GET_REGISTER_WIDE(registerNum));
                        paramNum++;
                        break;
                    case 'F':
                        ILOGV("param%d F %f", paramNum, (float)GET_REGISTER_FLOAT(registerNum));
                        break;
                    case 'D':
                        ILOGV("param%d D %lf", paramNum, (double)GET_REGISTER_DOUBLE(registerNum));
                        paramNum++;
                        break;
                    case 'L':
                        ...
                        tmpObj = (Object*)GET_REGISTER(registerNum);
                        if(strncmp(paramStr, "Ljava/lang/String;", strlen("Ljava/lang/String;")) == 0) {
                            ILOGV("param%d %s %s", paramNum, paramStr, dvmCreateCstrFromString((StringObject*)tmpObj));
                        } else {
                            if(tmpObj != NULL) {
                                ILOGV("param%d %s %p", paramNum, (tmpObj)->clazz->descriptor, tmpObj);
                            } else {
                                ILOGV("param%d (NULL) %p", paramNum, tmpObj);
                            }
                        }
                        break;
                    case '[':
                        tmpObj = (Object*)GET_REGISTER(registerNum);
                        paramStrIdx = deobfPrintParam(self, pc, curMethod, paramStrIdx+1, paramNum, (ArrayObject*)tmpObj);
                        break;
                    default:
                        ILOGV("param%d unknown parameter", paramNum);
                        break;
                    }
                    paramStrIdx++;
                }
                ...
                //ILOGV("args=%d {v%d, v%d, v%d, v%d, v%d}",
                //  vsrc1 >> 4, vdst & 0x0f, (vdst & 0x00f0) >> 4, (vdst & 0x0f00) >> 8, vdst >> 12, vsrc1 & 0x0f);
                ...
                ILOGV("paramCnt : %d, args %d", paramCnt, vsrc1 >> 4);
                ...
                paramStrIdx++;
                if(paramCnt != (vsrc1 >> 4)) {
                    ILOGV("param1 %s thisPtr", ((Object*)GET_REGISTER(vdst & 0x0f))->clazz->descriptor);
                    paramNum = 2;
                    paramCnt++;
                } else {
                    paramNum = 1;
                ...
                for(; paramNum <= paramCnt; ++paramNum) {
                    // ILOGV("deobfPararm %d", paramNum);
                    
                    switch(paramNum) {
                    case 5:
                        registerNum = vsrc1 & 0x0f;
                        break;
                    case 4:
                        registerNum = vdst >> 12;
                        break;
                    case 3:
                        registerNum = (vdst & 0x0f00) >> 8;
                        break;
                    case 2:
                        registerNum = (vdst & 0x00f0) >> 4;
                        break;
                    case 1:
                        registerNum = vdst & 0x0f;
                        break;
                    default:
                        ILOGV("paramNum Error : param%d", paramNum);
                    }
...
////haehyun
            ...
            if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0){
              if(gAndromonitormonitorOn == true) {
                    ...
                    if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
                        if((strncmp(methodToCall->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) != 0) ||
                           (strncmp(methodToCall->name, gAndromonitor.methodName, gAndromonitor.length_MN) != 0) ||
                           (strncmp(desc_v, gAndromonitor.methodDescriptor, gAndromonitor.length_MD) != 0) ){
                                gAndromonitormonitorOn = false;
                        }
                    }
                }
            ...
            if(self->threadId == gAndromonitor.threadNumber || gAndromonitor.threadNumber == 0) {
                ...
                if(gAndromonitor.currentMonitorMode == METHOD_MODE) {
                    if(gAndromonitormonitorOn == false){
                        if(strncmp(curMethod->clazz->descriptor, gAndromonitor.classDescriptor, gAndromonitor.length_CD) == 0){
                            if(strncmp(curMethod->name, gAndromonitor.methodName, gAndromonitor.length_MN) == 0){
                                if(strncmp(desc_vr, gAndromonitor.methodDescriptor, gAndromonitor.length_MD) == 0){
                                    gAndromonitormonitorOn = true;
                                }
                            }
                        }
                ...
                if(gAndromonitormonitorOn == true) {
                    gAndromonitor.isNative = false;

                    if(strstr(desc_v, ")Ljava/lang/String;")!=NULL){
                        tempstrObj = (StringObject*)retval.i;
                        ILOGV("retStr = %s", dvmCreateCstrFromString(tempstrObj));
                    }
                    else{
                    ...
                    if(gAndromonitor.extractInvokeOnly == false){
                        ILOGV("return from native %s->%s%s to %s->%s%s [fp=%p]",
                            methodToCall->clazz->descriptor, methodToCall->name, desc_v,
                            curMethod->clazz->descriptor, curMethod->name,
                            desc_vr, fp);
                    }
                    else{
                        ILOGV("return from native %s->%s%s to %s->%s%s ",
                            methodToCall->clazz->descriptor, methodToCall->name, desc_v,
                            curMethod->clazz->descriptor, curMethod->name,
                            desc_vr);
                    }
...
bail:
    ILOGV("Leaving interpreter loop");      // note "curMethod" may be NULL
