...
/////////////////////////////////////////////////////////////////////////
    ...
    if(strcmp(gAndromonitor.currentPackageName, gAndromonitor.packageName) == 0 && self->threadId == 1) {
    //if(strncmp(gAndromonitor.currentPackageName,gAndromonitor.packageName,strlen(gAndromonitor.currentPackageName)) == 0) {
    //   ALOGE("HaehyunDEBUG %s\n", gAndromonitor.packageName);
    //   ALOGE("HaehyunDEBUG %s\n", gAndromonitor.currentPackageName);
    // ALOGE("msec|interp in currentPackageName ");
    /* 
     * monitorMode 1 Full
     * monitorMode 2 Class
     * monitorMode 3 Method
     * monitorMode 4 API CALL
     */ 
        
             // ALOGE("packageName %s!!",gAndromonitor.packageName);
             // ALOGE("monitorMode %d!!",gAndromonitor.monitorMode);
             // ALOGE("classDescriptor %s!!",gAndromonitor.classDescriptor);
             // ALOGE("methodName %s!!",gAndromonitor.methodName);
             // ALOGE("methodDescriptor %s!!",gAndromonitor.methodDescriptor);
             // ALOGE("interp extractAPIinstructions %d!!",gAndromonitor.extractAPIinstructions?1:0);

        switch(gAndromonitor.monitorMode) {
            case 0:
                gAndromonitor.currentMonitorMode = 0;
                break;
            case 1:
                //ALOGE("msec|interp switch case 1");
                gDvm.executionMode = kExecutionModeInterpPortable;
                gAndromonitor.currentMonitorMode = FULL_MODE;
                break;
            case 2:
                //ALOGE("msec|interp switch case 2");
                gDvm.executionMode = kExecutionModeInterpPortable;
                gAndromonitor.currentMonitorMode = CLASS_MODE;
                break;
            case 3:
                //ALOGE("msec|interp switch case 3");
                gDvm.executionMode = kExecutionModeInterpPortable;
                gAndromonitor.currentMonitorMode = METHOD_MODE;
                break;
            case 4:
                //ALOGE("msec|interp switch case 4");
                gDvm.executionMode = kExecutionModeInterpPortable;
                gAndromonitor.currentMonitorMode = API_CALL_MODE;
                gAndromonitor.internalCode = false;
                break;
            default:
                break;
        }
    ...
    // if(gInterpPortable.codeCheck == false && self->threadId == 1){  /* main thread */
    //     // FILE *fp;
    //     // char fname[30], pname[50];
    //     // sprintf(fname, "/proc/%d/cmdline", getpid());
    //     // fp = fopen(fname, "r");
    //     // fgets(pname, sizeof(pname)-1,fp);
    //     // fclose(fp);

    //     setCurrentPackageModeAndName();
    //     /*
    //         gInterpPortable.currentPackageMode
    //         gInterpPortable.currentPackageName[100]

    //         #define NORMAL_MODE 0
    //         #define MONITOR_MODE 1
    //         #define PRINT_MODE 2
    //         #define SANITIZE_SERVER_CORECODE_MODE 3
    //         #define SANITIZE_DONGLE_SIGN_MODE 4
    //         #define SANITIZE_DONGLE_CORECODE_MODE 5
    //         #define SANITIZE_DONGLE_HASH_MODE 6
    //     */
    //     // if(gInterpPortable.currentPackageMode !=0){
    //     //    ALOGE("msec|package : %s | %d",gInterpPortable.currentPackageName,gInterpPortable.currentPackageMode);
    //     // }
    //     switch(gInterpPortable.currentPackageMode)
    //     {
    //         case 0:  //NORMAL_MODE
    //             break;
    //         case 1:  //MONITOR_MODE
    ...
    //         case 2:  //PRINT_MODE
    //             gDvm.executionMode = kExecutionModeInterpPortable;
    //             if(gDvm.myInit == false){
    //                 gDvm.myInit = true;
    //                 // connectServer();
    //                 // if(gInterpPortable.getCodeFromServer == false){
    //                 //     myClose(TAMPER_CONNECT_FAIL, self);
    //                 // }
    //                 gInterpPortable.currentPackageMode = 3;
    //                 myPrintf("%s", gInterpPortable.hash);
    //                 gInterpPortable.currentPackageMode = 2;
    //             }
    //             break;
    //         case 3:  //SANITIZE_SERVER_CORECODE_MODE
    //             gDvm.executionMode = kExecutionModeInterpPortable;
    //             if(gDvm.myInit == false){
    //                 gDvm.myInit = true;
    //                 // connectServer();
    ...
    //             }
    //             break;
    //         case 4:  //SANITIZE_DONGLE_SIGN_MODE
    //             if(gDvm.myInit == false){
    //                 gDvm.myInit = true;
    //                 signCheck(gInterpPortable.currentPackageName);

    //                 gInterpPortable.currentPackageMode = 3;
    //                 myPrintf("%s", gInterpPortable.hash);
    //                 gInterpPortable.currentPackageMode = 4;
    //             }
    //             break;
    //         case 5:  //SANITIZE_DONGLE_CORECODE_MODE
    //             break;
    //         case 6:  //SANITIZE_DONGLE_HASH_MODE
    //             if(gDvm.myInit == false){
    //                 gDvm.myInit = true;
    //                 hashCheck(gInterpPortable.currentPackageName);

    //                 gInterpPortable.currentPackageMode = 3;
    //                 myPrintf("%s", gInterpPortable.hash);
    //                 gInterpPortable.currentPackageMode = 6;
    //             }
    //             break;
    //         default:
    //             ALOGE("undefined execute mode");
    //             break;
    //     }
    //   //  if(strstr(pname, APPNAME)){
    //         // gDvm.executionMode = kExecutionModeInterpPortable;
    //         // gInterpPortable.currentPackageMode = MODE;
