...
//3.cmp
// int 
// nativeCodeExtractor(const char* filename){
//   FILE *fp;
//   char path[30];
//   char configPackageName[256];
//   char currentPackageName[256];
//   char configPath[50]="/data/andromonitor/config.txt";
//   char buf[256];
//   char *line;

//   sprintf(path, "/proc/%d/cmdline", getpid());
//   fp = fopen(path, "r");
//   fgets(currentPackageName, sizeof(currentPackageName)-1,fp);
//   fclose(fp);
...
//       }
//       line = strtok(buf,"=");
//       if(strcmp("packageName",line) == 0) {
//         if((line = strtok(NULL,"\n")) != NULL) {
//           strcpy(configPackageName,line);
//           break;
//         }
//       }
//     } 
//     fclose(fp);
//   }
//   ALOGE("MSEC dvmLoadNativeCode| filename = %s",filename);
//   ALOGE("MSEC dvmLoadNativeCode| configPackageName = %s",configPackageName);
//   ALOGE("MSEC dvmLoadNativeCode| currentPackageName = %s",currentPackageName);

//   if(strcmp(configPackageName,currentPackageName) == 0) {
//     //copycode
...
/////////////////andromonitor
    ...
    ALOGE("msec|dvmLoadNativeCode currentPackageName-> %s", gAndromonitor.currentPackageName);
    ALOGE("msec|dvmLoadNativeCode g.packageName-> %s", gAndromonitor.packageName);
    if(strcmp(gAndromonitor.currentPackageName, gAndromonitor.packageName) == 0){
        ALOGE("msec|dvmLoadNativeCode -> %s", pathName);
        logging("msec|dvmLoadNativeCode -> %s", pathName);
        if(gAndromonitor.extractFile == true){
            fileExtractor(pathName);
        }
    }
